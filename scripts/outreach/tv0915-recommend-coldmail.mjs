// 테크밸리(TechValley) Market & IT Business Research Intern(V173) recommend — 9/15 신규 게재 당일 발송.
// 9/15 Mia 게재 알림 → 풀 실측(scripts/tmp/tv0915-pool-measure.mjs) 후 유저 지시 "코어만".
// JD: HN/HCMC 하이브리드(리모트 협의) · 인턴 2개월 · 월 400만 동 · 베트남어+영어 필수 · 영문 CV
//     · 경영/경제/정보시스템 등 재학 or 갓졸업 · IT/AI 시장 리서치·경쟁사 분석·보고서 작성.
// 코어 게이트: 인턴 연령대(재학 2026+ 졸업예정 or 갓졸업 ≤1y) × 영어 시그널 × HN/HCMC/미기재
//              × (경영/경제/정보시스템 계열 전공 or BA·데이터분석·리서치 직군). 9/15 실측 202명(당일 겹침 7).
// cosmos0915 패턴: 1인1통(당일 recommend 기수신 제외, --gap-hours 완화) · 공개/비공개 프레임 · unsub 전역 제외.
//
//   node scripts/outreach/tv0915-recommend-coldmail.mjs                       # dry-run
//   node scripts/outreach/tv0915-recommend-coldmail.mjs --send [--max N] [--gap-hours N]
...
const JOB_ID = '4fb83b6e-32c7-4954-b0f6-cbdec4b749fe' // TechValley Market & IT Business Research Intern (V173)

// ── 대상 선정 (scripts/tmp/tv0915-pool-measure.mjs 와 동일 게이트) ──
...
// 코어만(9/15 유저 지시). 확장(텍스트 시그널 129명)은 지시 시 그룹 추가.
const GROUPS = [
  {
    gkey: 'core', camp: 'tv0915-recommend-research-core',
    label: { vi: 'Market & IT Business Research Intern', ko: '리서치 인턴 코어(경영/경제/IS 전공 or BA·리서치 직군)' },
    pick: (p) => (internAge(p) && enSignal(p) && (inHcmc(p) || inHanoi(p) || noLoc(p)) && (bizMajorRe.test(String(p.major || '')) || baRole(p)) ? score(p) : null),
  },
]

// ── 카피(vi 실발송) — 조건(영어 필수·영문 CV·전공·2개월 400만 동) 전부 명시해 자기선별 유도 ──
const COMPANY = 'TechValley'
const INITIAL = 'T'
const META_VI = 'Hybrid · Hà Nội / TP.HCM · Thực tập 2 tháng · 4tr ₫/tháng'
const INTRO = '<b>TechValley</b> — công ty công nghệ Hàn Quốc hoạt động trong lĩnh vực IT &amp; AI tại Việt Nam — đang tuyển <b>Market &amp; IT Business Research Intern</b> (thực tập 2 tháng, <b>Hà Nội / TP.HCM, Hybrid — có thể thỏa thuận remote</b>) qua FYI. Công việc: theo dõi xu hướng thị trường IT/AI Việt Nam, phân tích doanh nghiệp &amp; đối thủ cạnh tranh (cloud, SaaS, AI), tổng hợp dữ liệu và viết báo cáo nghiên cứu, slide cho ban lãnh đạo. Yêu cầu: <b>tiếng Việt &amp; tiếng Anh tốt</b> (viết + nói), tư duy phân tích, thành thạo <b>Excel/Sheets &amp; PowerPoint/Slides</b>, đang học hoặc mới tốt nghiệp ngành <b>Quản trị Kinh doanh, Kinh tế, Hệ thống thông tin</b> hoặc liên quan. Trợ cấp <b>4.000.000 ₫/tháng</b>, mentor trực tiếp từ các doanh nhân công nghệ Hàn–Việt, giấy chứng nhận &amp; thư giới thiệu khi hoàn thành (hỗ trợ số liệu &amp; dấu công ty cho báo cáo thực tập nếu cần). <b>Nộp CV bằng tiếng Anh.</b>'
const SUBJECT = {
...
const HOOK = 'Đội ngũ FYI đã xem xét toàn bộ hồ sơ đã đăng ký và <b>chọn bạn vào danh sách đề cử</b> cho vị trí dưới đây — chuyên ngành và tiếng Anh trong hồ sơ của bạn phù hợp với yêu cầu của vị trí này.'
const BENEFIT = {
...
async function main() {
  ...
  for (const { p, g, frame } of targets) {
    ...
    await sb.from('events').insert([{
      event: 'recommend_sent', page: '/scripts/tv0915-recommend-coldmail',
