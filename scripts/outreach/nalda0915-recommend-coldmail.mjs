// NALDA UI/UX Intern(R201) recommend — 9/15 신규 게재 당일 발송.
// 9/15 Len 게재 알림(Slack) → 풀봇 로직 실측(코어 228=Design 직군) 후 유저 지시 "코어만".
// JD: 다낭(리모트 가능) · 인턴 · 월 300~400만 동 · Figma 필수 · 포트폴리오 제출 필수
//     · web/app UI/UX 설계·Figma 프로토타입·모바일/태블릿/PC 반응형·usability 개선.
// 코어 게이트: 직군(position/desired_roles)=Design — poolEstimator 코어와 동일. 경력 컷 없음(인턴, 유저 "코어만" 지시).
// tv0915 패턴: 1인1통(당일 recommend 기수신 제외, --gap-hours 완화) · 공개/비공개 프레임 · unsub 전역 제외.
//
//   node scripts/outreach/nalda0915-recommend-coldmail.mjs                       # dry-run
//   node scripts/outreach/nalda0915-recommend-coldmail.mjs --send [--max N] [--gap-hours N]
...
const JOB_ID = 'd05325d9-137c-4bd7-bc8e-e5e522a775a9' // NALDA UI/UX Intern (R201)

// ── 대상 선정: poolEstimator 코어와 동일(직군=Design). 점수는 정렬용 디자인 시그널만 ──
...
const score = (p) => {
  ...
  let s = 0
  if (/figma/.test(t)) s += 2
  if (/(ui\/ux|ui\s*ux|uiux|ux\/ui)/.test(t)) s += 2
  if (/(portfolio|behance|dribbble)/.test(t)) s += 1
  if (/(prototype|wireframe|usability)/.test(t)) s += 1
...
const GROUPS = [
  {
    gkey: 'core', camp: 'nalda0915-recommend-uiux-core',
    label: { vi: 'UI/UX Intern', ko: 'UI/UX 인턴 코어(Design 직군)' },
    pick: (p) => (isDesign(p) ? score(p) : null),
  },
]

// ── 카피(vi 실발송) — Figma 필수·포트폴리오 필수·인턴 수당을 전부 명시해 자기선별 유도 ──
const COMPANY = 'NALDA'
const INITIAL = 'N'
const META_VI = 'Đà Nẵng · Có thể remote · Thực tập · 3–4tr ₫/tháng'
const INTRO = '<b>NALDA</b> — công ty công nghệ Hàn Quốc vận hành ứng dụng quản lý thời gian <b>Timing</b> — đang tuyển <b>UI/UX Intern</b> (<b>Đà Nẵng, có thể làm việc remote</b>) qua FYI. Công việc: thiết kế UI/UX cho dịch vụ web/app, thiết kế UI &amp; tạo prototype bằng <b>Figma</b>, thiết kế màn hình responsive cho mobile/tablet/PC, cải thiện tính khả dụng (usability) của dịch vụ. Yêu cầu: sử dụng được <b>Figma</b>, có năng lực thiết kế UI/UX cơ bản, <b>nộp kèm portfolio</b>. Ưu tiên: kinh nghiệm thiết kế UI/UX web/app, responsive web, chuyên ngành thiết kế hoặc kinh nghiệm dự án, từng phối hợp với developer. Trợ cấp <b>3–4 triệu ₫/tháng</b>. Công cụ làm việc: Figma, Slack.'
const SUBJECT = {
...
const HOOK = 'Đội ngũ FYI đã xem xét toàn bộ hồ sơ đã đăng ký và <b>chọn bạn vào danh sách đề cử</b> cho vị trí dưới đây — hồ sơ thiết kế của bạn phù hợp với yêu cầu của vị trí này.'
const BENEFIT = {
...
async function main() {
  ...
  if (!doSend) {
    for (const g of GROUPS) {
      ...
      for (const { p, s, frame } of rows.slice(0, 30))
        console.log(`  [${s}·${frame}] ${p.full_name} <${p.email}> · ${p.position || '?'} · ${(p.yoe_months ?? '?')}mo · ${p.location || '위치?'}`)
    }
  ...
  for (const { p, g, frame } of targets) {
    ...
    await sb.from('events').insert([{
      event: 'recommend_sent', page: '/scripts/nalda0915-recommend-coldmail',
