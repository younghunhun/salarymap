...
// ── 블랙리스트 동기화 — ops "Qualified Candidates" 스프레드시트의 'Blacklist' 탭 → candidate_blacklist ──
// 시트 컬럼: Company · Name · Email · Position · CV · Reason for blacklisting (9/23 기준, 헤더명으로 매핑).
// 이메일이 키. user_profiles.email 로 user_id 를 붙여두면 로그인 지원은 id 로, 익명·원탭 지원은 이메일로 막힌다.
// source='sheet' 행만 전량 재적재(삭제 후 삽입 — 시트가 원본). 어드민이 직접 넣은 source='admin' 행은 건드리지 않되,
// 같은 이메일이 시트에도 올라오면 시트 행이 이긴다(유니크 인덱스 충돌 방지로 admin 행을 먼저 지운다).
export const QC_SHEET_ID = '1jkHaMY6CM4b-Y_VQlqwULVujqdFPsAhISRVl2TSqfNo'
export const BLACKLIST_TAB = 'Blacklist'
...
export async function syncBlacklist() {
  ...
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: QC_SHEET_ID, range: `'${BLACKLIST_TAB}'!A1:Z` })
  const rows = res.data.values || []
  if (rows.length < 1) throw new Error('Blacklist 탭이 비어 있음')
  const MAP = { company: 'company', name: 'full_name', email: 'email', position: 'position', cv: 'cv_url', reason: 'reason' }
  const idx = {}
  rows[0].forEach((h, i) => { const k = String(h || '').trim().toLowerCase().replace(/ for blacklisting$/, ''); if (MAP[k]) idx[MAP[k]] = i })
  if (idx.email === undefined) throw new Error(`Blacklist 탭 헤더에 Email 없음: ${JSON.stringify(rows[0])}`)

  const syncedAt = new Date().toISOString()
  const byEmail = new Map()
  for (let i = 1; i < rows.length; i++) {
    const get = (f) => (idx[f] !== undefined ? String(rows[i][idx[f]] || '').trim() : '')
    const email = get('email').toLowerCase()
    if (!email.includes('@')) continue
    byEmail.set(email, { email, full_name: get('full_name') || null, company: get('company') || null, position: get('position') || null,
      cv_url: get('cv_url') || null, reason: get('reason') || null, source: 'sheet', synced_at: syncedAt })
  }
  const emails = [...byEmail.keys()]

  // user_id 매칭 — 이메일 대소문자 무시. 한 이메일에 프로필이 여럿이면 첫 번째.
  if (emails.length) {
    const { data: profs } = await fyi.from('user_profiles').select('id, email').in('email', emails)
    for (const p of profs || []) { const r = byEmail.get(String(p.email || '').toLowerCase()); if (r && !r.user_id) r.user_id = p.id }
    // 대소문자가 다른 프로필 이메일 보강 (in 은 exact match)
    const missing = emails.filter((e) => !byEmail.get(e).user_id)
    for (const e of missing) {
      const { data: p } = await fyi.from('user_profiles').select('id').ilike('email', e).limit(1).maybeSingle()
      if (p) byEmail.get(e).user_id = p.id
    }
  }

  const { error: delErr } = await fyi.from('candidate_blacklist').delete().eq('source', 'sheet')
  if (delErr) throw new Error(`blacklist delete: ${delErr.message}`)
  if (emails.length) {
    // 유니크(lower(email)) 충돌 방지 — 같은 이메일의 admin 행은 시트 행으로 대체
    const { error: delAdmErr } = await fyi.from('candidate_blacklist').delete().in('email', emails)
    if (delAdmErr) throw new Error(`blacklist delete admin dup: ${delAdmErr.message}`)
    const { error: insErr } = await fyi.from('candidate_blacklist').insert([...byEmail.values()])
    if (insErr) throw new Error(`blacklist insert: ${insErr.message}`)
  }
  return { total: emails.length, matched: [...byEmail.values()].filter((r) => r.user_id).length, syncedAt }
}
